...
namespace Microsoft.Maui.Media
{
    ...
    class MediaPickerImplementation : IMediaPicker
    {
        ...
        public MediaPickerImplementation(ICapturePicker? capturePicker = null)
        {
            ...
            if(capturePicker == null)
            {
                try
                {
                    ...
                    var nativePath = Path.Combine(AppContext.BaseDirectory, "runtimes", $"linux-{arch}", "native");
                    Console.WriteLine($"Using native path: {nativePath}");

                    // Add native path to environment variables
                    var pathVar = Environment.GetEnvironmentVariable("PATH") ?? "";
                    if (!pathVar.Contains(nativePath))
                        Environment.SetEnvironmentVariable("PATH", nativePath + Path.PathSeparator + pathVar);

                    // Optionally: preload cvextern to avoid DllNotFoundException
                    NativeLibrary.Load(Path.Combine(nativePath, "libcvextern.so"));
                }
                catch (Exception e)
                {
        ...
        public class CapturePhotoWindow : AnimatableWindow
        {
                ...
                : base()
            {
                ...
                _closeButton.Click += (_, _) => Close(null);
                ...
                var buttonPanel = new StackPanel
                {
                    ...
                    HorizontalAlignment = HorizontalAlignment.Center,
                    Children = { _captureButton, _acceptButton, _cancelButton, _closeButton },
                    Spacing = 10,
            ...
            {
                _capture = new VideoCapture("http://192.168.1.109:56000/mjpeg"); 
        ...
        public class CaptureVideoWindow : AnimatableWindow
        {
                ...
                : base()
            {
                ...
                var buttons = new StackPanel
                {
                    ...
                    Spacing = 10,
                    Margin = new Thickness(10),
                    Children = { _recordButton, _acceptButton, _cancelButton, _closeButton }
                };
